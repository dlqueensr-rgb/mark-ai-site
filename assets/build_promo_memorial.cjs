...
const EMAIL_URL = 'https://raw.githubusercontent.com/dlqueensr-rgb/mark-ai-site/main/assets/promo_memorial_email.html';
const CAMPAIGN_NAME = 'Mark AI — Memorial Day Sale (Residential)';
const TERMS = 'Minimum 2 hours labor. Offer ends Mon Jun 7, 2027. Labor only. Cannot be combined with other offers or membership pricing.';
const SMS_BODY = "Hi {firstName}, happy Memorial Day! {offerAmount} off labor this weekend in honor of those who served — minimum 2 hrs, book by Mon Jun 7. Call {campaignPhone}. — {companyName}";
const CONFIG = {
  send_window: { tz:'America/Los_Angeles', days:[1,2,3,4,5,6,7], start_hour:9, end_hour:19 },
  tracking_phone: '(916) 234-0885',
  st_campaign_name: 'Mark AI — Memorial Day Sale',
  offer: { type:'percent', amount:'20', expires_days:10, terms:TERMS }
};
const PROMO_START='2027-05-28'; const PROMO_END='2027-06-01';
...
async function main(){
  ...
  try{
    const dup=await client.query("SELECT id FROM drip_campaigns WHERE franchise_id=$1 AND name ILIKE '%Memorial Day Sale%'",[FRANCHISE_ID]);
    ...
    const smsT=await client.query("INSERT INTO drip_templates(franchise_id,name,channel,subject,body)VALUES($1,$2,'sms',NULL,$3)RETURNING id",[FRANCHISE_ID,'Memorial Day Sale SMS — Residential',SMS_BODY]);
    const emT=await client.query("INSERT INTO drip_templates(franchise_id,name,channel,subject,body)VALUES($1,$2,'email',$3,$4)RETURNING id",[FRANCHISE_ID,'Memorial Day Sale Email — Residential','Memorial Day Sale — {offerAmount} off labor this weekend, {firstName}',emailBody]);
    ...
    await client.query("INSERT INTO drip_autoenroll_map(franchise_id,segment,campaign_id,months,channel_need,project_label,daily_limit,enabled,notes,customer_type,promo_start_date,promo_end_date)VALUES($1,'all_dialable',$2,NULL,'sms','your home',9999,false,'Memorial Day Sale — promo May28-Jun1 2027','residential',$3,$4)",[FRANCHISE_ID,cid,PROMO_START,PROMO_END]);
