...
const EMAIL_URL = 'https://raw.githubusercontent.com/dlqueensr-rgb/mark-ai-site/main/assets/promo_blackfriday_email.html';
const CAMPAIGN_NAME = 'Mark AI — Black Friday Sale (Residential)';
const TERMS = 'Minimum 2 hours labor. Offer ends Mon Dec 7, 2026. Labor only. Cannot be combined with other offers or membership pricing.';
const SMS_BODY = "Hi {firstName}, it's our Black Friday Sale — {offerAmount} off labor, our biggest discount of the year. Min 2 hrs, book by Mon Dec 7. Call {campaignPhone}. — {companyName}";
const CONFIG = {
  send_window: { tz:'America/Los_Angeles', days:[1,2,3,4,5,6,7], start_hour:9, end_hour:19 },
  tracking_phone: '(916) 234-0885',
  st_campaign_name: 'Mark AI — Black Friday Sale',
  offer: { type:'percent', amount:'20', expires_days:12, terms:TERMS }
};
const PROMO_START='2026-11-25'; const PROMO_END='2026-12-01';
...
async function main(){
  ...
  try{
    const dup=await client.query("SELECT id FROM drip_campaigns WHERE franchise_id=$1 AND name ILIKE '%Black Friday Sale%'",[FRANCHISE_ID]);
    ...
    if(!COMMIT){
      ...
      console.log('  offer: PERCENT 20% off labor, expires 12 days, min 2hrs');
      console.log('  window:',PROMO_START,'..',PROMO_END,'/ blast 9999/day');
      console.log('\nRe-run with --commit');process.exit(0);
    }
    await client.query('BEGIN');
    const smsT=await client.query("INSERT INTO drip_templates(franchise_id,name,channel,subject,body)VALUES($1,$2,'sms',NULL,$3)RETURNING id",[FRANCHISE_ID,'Black Friday Sale SMS — Residential',SMS_BODY]);
    const emT=await client.query("INSERT INTO drip_templates(franchise_id,name,channel,subject,body)VALUES($1,$2,'email',$3,$4)RETURNING id",[FRANCHISE_ID,'Black Friday Sale Email — Residential','Black Friday Sale — {offerAmount} off labor this week, {firstName}',emailBody]);
    ...
    await client.query("INSERT INTO drip_autoenroll_map(franchise_id,segment,campaign_id,months,channel_need,project_label,daily_limit,enabled,notes,customer_type,promo_start_date,promo_end_date)VALUES($1,'all_dialable',$2,NULL,'sms','your home',9999,false,'Black Friday Sale — promo Nov25-Dec1 2026','residential',$3,$4)",[FRANCHISE_ID,cid,PROMO_START,PROMO_END]);
